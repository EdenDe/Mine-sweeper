...
function removeGhost(ghostPos) {
  var inx = gGhosts.findIndex(
    (ghost) =>
      ghost.location.i === ghostPos.i && ghost.location.j === ghostPos.j
  )
  if (inx === -1) return
...
function getGhostHTML(ghost) {
  return `<i class="ghost ${GHOST}" style='color: ${
    gPacman.isSuper ? FEAR_COLOR : ghost.color
  }'></i>`
}
